...
\newcommand{\coll}{\mathrel{\text{coll.}}}
...
\begin{tikzpicture}[thick,scale=1.0,
    p/.style={circle, draw, fill=white,
        ...
        \draw
                (30:0.5) node[p]{} -- (65:1.3) node[p]{}
                (30:0.5)  -- (25:1.3) node[p]{}
                (65:1.3)  -- (25:1.3)
            
                (150:0.5) node[p]{} -- (155:1.3) node[p]{}
                (150:0.5)  -- (115:1.3) node[p]{}
                (155:1.3)  -- (115:1.3)
            
            (90:0.5) node[p]{}
            (90:-0.5) node[p]{}
            (115:1.3) -- (65:1.3) -- (90:0.5) -- (115:1.3)
            (90:0.5) -- (90:-0.5)
            (350:1) node[p]{}
            (190:1) node[p]{}
            (155:1.3) -- (190:1) -- (270:0.5) -- (350:1) -- (25:1.3)
            (150:0.5) -- (350:1)
            (30:0.5) -- (190:1)
            ;
    \end{tikzpicture}
    ...
    p/.style={circle, draw, fill=white,
        ...
        \draw
            \foreach \x in {0,60,120,180,240,300} {
                (\x:1.3) node[p]{} -- (\x+60:1.3) node[p]{}
            }
	(0,0) node[p]{} -- (270:0.5) node[p]{}
	(0,0) -- (45:0.5) node[p]{} -- (300:1.3)
	(0:0) -- (135:0.5) node[p]{} -- (240:1.3)
	(180:1.3) -- (270:0.5) -- (0:1.3)
	(60:1.3) -- (135:0.5)
	(120:1.3) -- (45:0.5);
    \end{tikzpicture}
    \qquad
...
% Apparently, \appendix does not work properly.
% Thus, we set \thesubsection by hand
\appendix
\clearpage
...
\subsection{Second cross-product unembeddability proof}
...
\begin{tikzpicture}[thick,scale=1.4,
    p/.style={circle, draw, fill=white,
        ...
        \draw
            ...
            (45+90:0.5) -- (45+90+180:0.5)
            (90:0.9) node[above] {$x$}
            (270:0.9) node[below] {$w$}
            (15:1.3) node[right] {$y$}
            (-15:1.3) node[right] {$v$}
            (45:0.5) node[below] {$z$}
            (-45:0.5) node[above] {$p_5$}
            (135:0.5) node[below] {$p_1$}
            (-135:0.5) node[above] {$p_4$}
            (165:1.3) node[left] {$p_2$}
            (-165:1.3) node[left] {$p_3$}
        ;
    \end{tikzpicture}}

\par\noindent\parbox{\textwidth}{%
\begin{proposition}\label{prop:unemb-10-1}
The graph on the right is not embeddable.
\end{proposition}}
\begin{myproof}
    Suppose it is embeddable.
    Note (the point associated to) $p_5$
        is orthogonal to (the points associated to) $v$ and~$w$.
    As~$v \neq w$, we see~$p_5$ is collinear
    to cross-product~$v \times w$.
    Similarly, we derive
    \begin{align*}
        p_1 &\coll x \times (v \times w) &
        p_4 &\coll w \times z \\
        p_2 &\coll x \times (x \times v \times w) &
        p_3 & \coll w \times (w \times z) \\
        \omit\rlap{$
        p_3 \coll (w \times z) \times (x \times (x \times (v \times w)))$}
    \end{align*}
    And thus
    \begin{equation}
        (w \times (w \times z)) \times
        ((w \times z) \times (x \times (x\times (v \times w)))) =0.
            \label{eq:coll2}
    \end{equation}
    By rotating the embedding,
        we may assume without loss of generality
            that we have~$z=(0,0,1)$, $y=(0,1,0)$ and~$x=(1,0,0)$.
    Write
\begin{align*}
    v&=\Bigl(v_1,0,\sqrt{1-v_1^2}\Bigr) &
        w &= \Bigl(w_1,w_2,\sqrt{1-w_1^2-w_2^2}\Bigr).
\end{align*}
Now, equation~\ref{eq:coll2} becomes
\begin{align*}
    w_2^2 \Bigl(v_1-\sqrt{1-v_1^2}w_1\sqrt{1-w_1^2-w_2^2}\Bigr) &= 0 \\
    w_1w_2\Bigl(\sqrt{1-v_1^2}w_1 \sqrt{1-w_1^2 - w_2^2} - v_1 \Bigr) &= 0.
\end{align*}
Note~$w_1 \neq 0$,
for otherwise~$w$ would be orthogonal to~$x$,
which would imply~$w = y$, quod non.
Similarly~$w_2 \neq 0$, for otherwise~$w$ would be orthogonal to~$y$
and then~$v = p_4$, quod non.
And finally, $v_1 \neq 0$, for otherwise~$v$ would be orthogonal to~$x$
and then~$x = p_5$, quod non.
It follows~$v_1 - \sqrt{1-v_1^2}  w_1 \sqrt{1-w_1^2 - w_2^2} = 0$.
Thus $v_1 = \sqrt{1-v_1^2}  w_1 \sqrt{1-w_1^2 - w_2^2}$.
As $v$ is orthogonal to~$w$,
we have~$-v_1 w_1 = \sqrt{1-v_1^2}\sqrt{1-w_1^2-w_2^2}$.
Substituting this in the previous equation,
we get~$v_1 = -v_1w_1^2$.
That is: $w_1^2 = -1$, which is absurd.
Apparently, the graph cannot be embeddable.
\end{myproof}
