...
\begin{proposition}[\cite{aow11}]\label{prop:ks-conn}
...
\begin{proposition}\label{prop:ks-enoughtriangles}
    In a minimal Kochen-Specker graph,
        every node is part of a triangle.
\end{proposition}
\begin{proof}
...
\end{proof}
There are only TODO squarefree non-isomorphic 
graphs on 17 vertices with minimal vertex degree 3
where every node is part of a triangle.
Unfortunately,
we could not find an efficient algorithm
to restrict the enumeration of graphs
to those where every node is part of a triangle.
