...
Suppose we are given a connected but not edge-biconnected graph~$G$.
Then we can decompose
this graph into its edge-biconnected components.  A edge-biconnected components
is a maximal subgraph that is edge-biconnected.
We can consider the graph of edge-biconnected components~$B(G)$:
two edge-biconnected components are adjacent if there is an edge
between a vertex in one component and a vertex in the other.
Note that there can at most be one edge between 
the vertices of edge-biconnected components.
$B(G)$~does not contain loops: if it would
then the union of the edge-biconnected components in the graph,
would be itself edge-biconnected.  Thus~$B(G)$ is a tree.

Furthermore suppose, reasoning towards contradication,
that~$G$ is a minimal Kochen-Specker graph.
Now consider a leaf~$A$ in the edge-biconnected component graph~$B(G)$.
That is, $A$ is a edge-biconnected component that is connected to only
one other edge-biconnected component.
...
we see~$A'$ is a Kochen-Specker graph that is not edge-biconnected.
...
\begin{proof}
    ...
    \item Suppose~$a_1 \neq a_2$ and~$b_1 = b_2$.
        ...
        \begin{enumerate}
            ...
            Thus~$\#A \geq 16$.
            Consequently~$\#G \geq 32$. Contradiction with minimality.
            ...
            Hence~$\#A\geq 17$. (?) % FIXME check and ref
    ...
    \item Apparently~$a_1 \neq a_2$ and~$b_1 \neq b_2$.
        ...
        Thus both~$\#A \geq 16$ and~$\#B \geq 16$.
        Hence~$\#G \geq 32$. Contradiction with minimality. \qedhere
    \end{enumerate}
