...
\begin{dfn}
   ...
   The \keyword{type of~$(v_1,\ldots,v_n)$ (in~$G$)}
   is the set of all possible ways it can be colored;
   that is: $t_G(v_1, \ldots, v_n) := \{ (c(v_1), \ldots, c(v_n));
                \ c \in C(G) \}.$ 
    A type of~$n$ vertices is
    called an~\keyword{$n$-type}.
\end{dfn}

Note that~$v$ has the~$1$-type~$(1)$ in~$G$ if and only if
it has fixed color~$1$ in~$G$.
Just like points with fixed color,
we are interested in types,
because they do not occur in small graphs.
...
\begin{comp}\label{comp:types}
    ...
    \end{tikzpicture}
    This occurance is called trivial, for the two vertices are adjacent.
\end{comp}
